...
export async function updateUserActivity(email, activityType, activityDetails) {
  try {
    ...
    const userActivity = { ...defaultActivity, ...user.userActivity };
    ...
    switch (activityType) {
      ...
      case 'playForFun':
        if (!['win', 'lost', 'restart'].includes(activityDetails.result)) {
          return { success: false, message: 'Invalid result for playForFun'};
        }
        updateData = {
          $push: {
      ...
      case 'playForReal':
        if (!['easy', 'medium', 'hard'].includes(activityDetails.level)) {
          return { success: false, message:'Invalid level for playForReal'};
        }
        if (!['win', 'lost', 'restart'].includes(activityDetails.result)) {
          return { success: false, message: 'Invalid result for playForReal'};
        }
        updateData = {
          $push: {
