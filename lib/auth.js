...
export async function updateUserActivity(email, activityType, activityDetails) {
  try {
    ...
    // Ensure all required activity fields exist, initialize them if necessary
    user.userActivity = user.userActivity || {}; // Ensure userActivity exists

    // Initialize individual activity arrays if they don't exist
    user.userActivity.signIn = user.userActivity.signIn || [];
    user.userActivity.signOut = user.userActivity.signOut || [];
    user.userActivity.playForFun = user.userActivity.playForFun || { win: [], lost: [], restart: [] };
    user.userActivity.playForReal = user.userActivity.playForReal || {
    ...
    switch (activityType) {
      case 'signIn':
        updatedUserActivity = {
          ...user.userActivity,
          signIn: [...user.userActivity.signIn, { ...activityDetails, date: new Date() }],
        };
        break;
      case 'signOut':
        updatedUserActivity = {
          ...user.userActivity,
          signOut: [...user.userActivity.signOut, { ...activityDetails, date: new Date() }],
        };
        break;
      case 'playForFun':
        updatedUserActivity = {
          ...user.userActivity,
          playForFun: {
            ...user.userActivity.playForFun,
            [activityDetails.result]: [
              ...(user.userActivity.playForFun[activityDetails.result] || []), // Ensure it's an array
              { ...activityDetails, date: new Date() },
            ],
          },
        };
        break;
      case 'playForReal':
        updatedUserActivity = {
          ...user.userActivity,
          playForReal: {
            ...user.userActivity.playForReal,
            [activityDetails.level]: {
              ...user.userActivity.playForReal[activityDetails.level],
              [activityDetails.result]: [
                ...(user.userActivity.playForReal[activityDetails.level][activityDetails.result] || []),
                { ...activityDetails, date: new Date() },
              ],
            },
          },
    ...
    console.log(updatedUserActivity);
    ...
    const result = await db.collection('users').updateOne(
      ...
      {
        $set: {
          'userActivity': updatedUserActivity, // Replace userActivity with the updated data
        },
      }
    );
    ...
    if (result.modifiedCount === 1) {
      ...
      return {
        success: false,
        message: 'No changes to user activity',
      };
    }
  } catch (error) {
    console.error('Error updating voucher:', error);
