...
// Purchase Vouchers
export async function purchaseVouchers(userId, voucherDetails) {
  try {
    ...
    // Prepare the purchase data with a random ID
    const newPurchaseData = {
      purchaseId: uuidv4(), // Generate a unique ID for the purchase
      vouchers: voucherDetails.map(v => ({
        id: v._id,
        title: v.title,
        amount: v.amount,
    ...
    if (!userPurchaseHistory) {
      // If this is the user's first purchase, add user information to the main record
      const newUserInfo = {
      ...
      // Insert the user info into the voucherPurchaseHistory collection for first-time users
      const result = await db.collection('voucherPurchaseHistory').insertOne({
        ...newUserInfo,
        purchases: [newPurchaseData], // Add purchase history
      });
      ...
      // If the user already has a purchase history, just update the purchases array
      const result = await db.collection('voucherPurchaseHistory').updateOne(
        { userId: new ObjectId(userId) }, // Match by userId
        { $push: { purchases: newPurchaseData } }, // Push new purchase data to the `purchases` array
    ...
    console.error('Error saving purchase record:', error.message);
...
export async function getPurchaseByUserId(userId) {
  try {
    // Find the purchase history for the given userId
    const purchaseHistory = await db.collection('voucherPurchaseHistory').findOne(
      { userId: new ObjectId(userId) }, // Match by userId
      { projection: { purchases: 1, _id: 0 } } // Return only the purchases array
    );

    if (!purchaseHistory) {
      return {
        success: false,
        message: 'No purchase history found for the given user ID.',
      };
    }
    ...
    return {
      ...
      data: purchaseHistory.purchases, // Return the purchases array
    };
  } catch (error) {
...
export async function getAllPurchaseRequest(itemsPerPage, pageNumber, searchQuery = "") {
  try {
    // Build the search criteria
    const searchCriteria = searchQuery
      ? {
          $or: [
            { 'name': { $regex: searchQuery, $options: 'i' } }, // Search by userName
            { 'email': { $regex: searchQuery, $options: 'i' } }, // Search by email
            { 'mobile': { $regex: searchQuery, $options: 'i' } }, // Search by mobile
          ],
        }
      : {};

    // Get the total count of purchase histories matching the search criteria
    const totalPurchaseHistories = await db
      .collection('voucherPurchaseHistory')
      .countDocuments(searchQuery ? searchCriteria : {});

    if (totalPurchaseHistories === 0) {
      return {
        success: false,
    ...
    // Fetch the purchase histories from the database with pagination and search criteria
    const purchaseHistories = await db
      .collection('voucherPurchaseHistory')
      .find(searchQuery ? searchCriteria : {})
      .skip(skip) // Skip documents based on the page number
      .limit(itemsPerPage) // Limit the number of documents per page
    ...
    return {
      ...
      pagination: {
        ...
        totalPages: Math.ceil(totalPurchaseHistories / itemsPerPage),
      },
    };
  } catch (error) {
    console.error('Error retrieving all purchase histories:', error.message);
    return { success: false, message: error.message };
  }
}
